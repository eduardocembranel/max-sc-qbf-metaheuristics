...
class AbstractGRASP(ABC, Generic[E]):
    ...
    def __init__(self, seed: int, target: float, obj_function: Evaluator[E], alpha: float, maxTimeSec: int, 
    ...
    def solve(self) -> Solution[E]:
        ...
        itr = 1
        while (self.secondsFrom(start_time) < self.maxTimeSec and self.best_sol.cost < self.target):
            ...
            if self.best_sol.cost < self.sol.cost:  # Maximization
                ...
                if self.verbose:
                    print(f"(Iter. {itr}) (time: {self.secondsFrom(start_time):.2f}) BestSolCost = {self.best_sol.cost}")
    ...
    def secondsFrom(self, start_time: float) -> float:
