...
public class TSSCQBF extends AbstractTS<Integer> {
    ...
    public TSSCQBF(
        ...
        super(seed, target, new SCQBFInverse(filename), tenure, maxTimeSeconds, enableDiversification, enableIntensification);
    ...
    */
    ...
    private Solution<Integer> firstImprovingMove() {
        ...
        List<Integer> indices = IntStream.range(0, CL.size())
                .boxed()
                .collect(Collectors.toList());
        Collections.shuffle(indices, this.rng);
        ...
        indices = IntStream.range(0, sol.size())
                .boxed()
                .collect(Collectors.toList());
        ...
        List<Integer> inIndices = IntStream.range(0, CL.size())
                .boxed()
                .collect(Collectors.toList());
        Collections.shuffle(inIndices, rng);

        List<Integer> outIndices = IntStream.range(0, sol.size())
                .boxed()
                .collect(Collectors.toList());
        Collections.shuffle(outIndices, rng);

        // Evaluate exchanges
        for (int iIn : inIndices) {
            Integer candIn = CL.get(iIn);
            for (int iOut : outIndices) {
                Integer candOut = sol.get(iOut);
                Double deltaCost = ObjFunction.evaluateExchangeCost(candIn, candOut, sol);
                if ((!TL.contains(candIn) && !TL.contains(candOut)) || sol.cost+deltaCost < bestSol.cost) {
                    //if this move improves the solution
                    if (deltaCost < 0) {
