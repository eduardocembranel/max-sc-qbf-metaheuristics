...
public abstract class AbstractTS<E> {
    ...
    public Solution<E> solve() {
        ...
        int it = 1;
        while (getElapsedSecs(start) < maxTimeSeconds && bestSol.cost > target) {
            neighborhoodMove();
            updateVarFrequency();
            ...
            if (bestSol.cost > sol.cost) {
                ...
                if (bestSol.cost <= target || getElapsedSecs(start) >= maxTimeSeconds) {
                    break;
                }
    ...
    private boolean checkDiversificationTrigger(int iterationsSinceLastImprove, int countDiversifications) {
        int[] diversifyAt = {50, 500, 2000, 4000, 6000, 8000, 10000, 12000, 14000, 16000, 18000};
        double[] diversifyPercents = {0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02};
