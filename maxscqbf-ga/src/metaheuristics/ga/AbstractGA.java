...
public abstract class AbstractGA<G extends Number, F> {
	...
	protected Integer totalGenerations;
	protected Integer bestSolGeneration;
	protected Double bestSolTime;
	protected Boolean usedLHS;
	protected Boolean usedUC;
	protected Double usedTime;
	...
	/**
	 * Makes sure children are feasible solutions.
	 * 
	 * @param child1
	 *        	The first child generated from the parents.
	 * @param child2
	 * 	  		The second child generated from the parents.
	 * @param parent1
	 * 		  	The first parent of the chromosome.
	 * @param parent2
	 * 		  	The second parent of the chromosome.
	 */
	protected abstract void fixChildren(Chromosome child1, Chromosome child2);
	 ...
	 *            The mutation rate.
	 */
	public AbstractGA(Evaluator<F> objFunction, Integer seed, Integer generations, Integer popSize, Double mutationRate) {
	 ...
	 */
	public Solution<F> solve(Boolean useLHS, Boolean useUC, Integer timeLimit, double target) {
		...
		Population population = initializePopulation(useLHS);

		bestChromosome = getBestChromosome(population);
		bestSol = decode(bestChromosome);

        var tGen0 =  ((double) (System.currentTimeMillis() - startTime)) / 1000.0;
		System.out.println("gen=0 bestSolVal=" + bestSol.cost + " t=" + tGen0);
		...
		int g = 1;
		while (System.currentTimeMillis() < endTime && bestSol.cost < target) {

			Population parents = selectParents(population);

			Population offsprings = crossover(parents, useUC);
			...
			if (fitness(bestChromosome) > bestSol.cost) {
				bestSol = decode(bestChromosome);
				bestSolGeneration = g;
				bestSolTime = ((double) (System.currentTimeMillis() - startTime)) / 1000.0;
				if (verbose)
                    System.out.println("gen=" + g + " bestSolVal=" + bestSol.cost + " t=" + bestSolTime);
			}
			g++;
		}
		totalGenerations = g;
	 ...
	 */
	protected Population initializePopulation(boolean useLHS) {

        if (useLHS) {
            return initializePopulationLHS();
        }
    ...
    protected abstract Population initializePopulationLHS();
	 ...
	 */
	protected Population crossover(Population parents, boolean useUC) {
		...
		for (int i = 0; i < popSize; i = i + 2) {
			...
			fixChildren(offspring1, offspring2);
	...
	protected Population crossoverUniform(Population parents) {
		...
		for (int i = 0; i < popSize; i = i + 2) {
			...
			int halfSize = chromosomeSize / 2;
			for (int m = 0; m < halfSize; m++) {
				mask.add(1);
			}
			for (int m = 0; m < chromosomeSize - halfSize; m++) {
				mask.add(0);
			}
			Collections.shuffle(mask, rng);
	...
	public String getStats() {
		return bestSol.cost + ";" + usedTime + ";" + popSize + ";" + mutationRate + ";" + (usedLHS ? "LHS" : "noLHS") +  ";"  + (usedUC ? "UC" : "noUC") +  ";" + bestSolTime + ";" + bestSolGeneration + ";" + totalGenerations + "\n";
	}
}
