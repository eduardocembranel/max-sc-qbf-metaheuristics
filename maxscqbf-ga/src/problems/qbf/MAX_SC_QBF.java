...
public class MAX_SC_QBF implements Evaluator<Integer> {
	...
	@Override
	public ArrayList<Integer> evaluateSetCoverage(Solution<Integer> sol) {
		ArrayList<Integer> coverage = new ArrayList<>(size);
		for (int i = 0; i < size; i++) {
			coverage.add(0);
		}

		for (Integer elementIdx : sol) {
			BitSet covers = coverBits[elementIdx];
			for (int coveredIdx = covers.nextSetBit(0); coveredIdx >= 0; coveredIdx = covers.nextSetBit(coveredIdx + 1)) {
				coverage.set(coveredIdx, coverage.get(coveredIdx) + 1);
			}
		}
	...
	@Override
	public boolean elementInSet(Integer elem, Integer set) {
		return coverBits[elem].get(set) ? true : false;
	}
